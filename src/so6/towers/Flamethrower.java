...
public class Flamethrower extends Tower {
    ...
    static {
        try {
            Shop.getElementList().add(new ShopElement(200, new Flamethrower(null)));
    ...
    public Flamethrower(IntVec2 pos) throws IOException {
        super(pos);
    ...
    public void update(Game game, float t, float dt) {
        ...
        if(t - tLastShot > 1.0f) {
            ...
            try {
                game.getProjectiles().add(new Projectile("fireball", this, target));
            } catch (IOException e) {
                e.printStackTrace();
            }
